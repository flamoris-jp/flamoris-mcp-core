...
partial-frame read and write limits. Idle time before the next frame is unbounded;
revocation and shutdown cancellation remain immediate. JSON depth, pending request
IDs, tool count, active requests and deadlines are bounded. Notifications have no
response lifetime and are read
